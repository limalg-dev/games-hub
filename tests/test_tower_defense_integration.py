...
class TestTowerUpgrade:
    ...
    def test_upgrade_increases_damage(self, game):
        ...
        success, msg, tower = game.place_tower(9, 4, TowerType.ARCHER)
        assert success
...
class TestTowerPlacement:
    ...
    def test_place_on_occupied_cell(self, game):
        ...
        game.place_tower(9, 4, TowerType.ARCHER)
        success, msg, _ = game.place_tower(9, 4, TowerType.BOMB)
        assert not success
...
async def test_tower_defense_upgrade_with_branch_api():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        ...
        # Place tower
        place = await ac.post(f"/tower-defense/games/{game_id}/place-tower", json={"x": 9, "y": 4, "tower_type": "archer"})
